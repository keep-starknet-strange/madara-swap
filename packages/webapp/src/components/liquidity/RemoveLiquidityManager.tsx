...
const RemoveLiquidityManager = ({ pool }: RemoveLiquidityManagerProps) => {
  ...
  const { removeLiquidity, updateRemoveLiquidityQuote, currentRemoveLiquidityQuote } = usePool();
  ...
  function removeLiquidityValidationReturn(validate: boolean) {
    ...
    if (validate && account && localRemoveLiquidityQuote) removeLiquidity(localRemoveLiquidityQuote, account);
  }
  ...
  const renderRemoveLPBody = () => {
    return (
      <>
        <Flex direction="column" align="flex-start" justify="flex-start" mb={4} color="whiteAlpha.600" w="50%">
