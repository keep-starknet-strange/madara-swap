import { Flex, Table, Tbody, Td, Text, Th, Thead, Tooltip, Tr } from "@chakra-ui/react";
  ...
  const sortPools = (poolsToSort: PoolState[], sortType: SortType, _sortDirection: SortDirection) => {
    return poolsToSort.sort((pool0, pool1) => {
      ...
      switch (sortType) {
        ...
        default:
          sortResult = parseFloat(pool0.liquidityUSD) < parseFloat(pool1.liquidityUSD);
          break;
      }
    ...
    return (
      <Flex direction="row" align="center" justify="flex-start">
        ...
        <Flex direction="column" ml={2}>
          ...
          <Text color="whiteAlpha.600" mt={1} fontSize="11px">
            {pool.name}
          </Text>
        </Flex>
      </Flex>
        ...
        {sortedPools &&
          sortedPools.map((pool: PoolState) => (
            <Tr key={`pool-${pool.address}`}>
              ...
              {/* <Td>x</Td> */}
              <Td isNumeric>${currencyFormatter(parseFloat(pool.liquidityUSD))}</Td>
              <Td isNumeric>
                {currencyFormatter(parseFloat(formatUint256ToReadableString(pool.reserveToken0, pool.token0.decimals)))}{" "}
                {pool.token0.symbol}
              </Td>
              <Td isNumeric>
                {currencyFormatter(parseFloat(formatUint256ToReadableString(pool.reserveToken1, pool.token1.decimals)))}{" "}
                {pool.token1.symbol}
              </Td>
