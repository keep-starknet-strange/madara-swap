...
// Check if there is a user input errors
// Basically, is input amount <= balance of token
export const getInputsErrorForAddLiquidity = (
  ...
  // Pool no exist
  // TODO remove this condition once dynamic contract creation is available
  if (!pool && token0 && token1) {
    return "Pool not exist";
  }

  const notEnoughAmountToken0 = isEnoughBalance(token0, amountToken0);
  const notEnoughAmountToken1 = isEnoughBalance(token1, amountToken1);

  // Error token0 not enough balance
  if (notEnoughAmountToken0) {
    return notEnoughAmountToken0;
  }

  // Error token1 not enough balance
  if (notEnoughAmountToken1) {
    return notEnoughAmountToken1;
  }
  return undefined;
};
